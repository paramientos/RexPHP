...
class str
{
     ...
     * @return type
     */
    public function freq($url, $json = false)
    {
    ...
    {
        return preg_replace_callback("|(\d+)|", [get_class(), '_inc'], $val);
    }

    public function _inc($matches)
    {
        if (isset($matches[1])) {
     ...
     * @return type
     */
    public function ngrams($word, $min_gram_length = 2)
    {
        $ngrams = [];
        $word = trim($word);
        $len = strlen($word);
     ...
     * @return type
     */
    public function remove_duplicated($vals, $json = false)
    {
        $list = [];
     ...
     * @return type
     */
    public function string_limiter($string, $limit, $pad = '...')
    {
     ...
     * @return type
     */
    public function zero_pad($value, $padding)
    {
        return str_pad($value, $padding, '0', STR_PAD_LEFT);
    }

    public function fake_data($length = 10)
    {
     ...
     * @return type
     */
    public function generate_string($type = 'alnum', $len = 6)
    {
        switch ($type) {
    ...
    {
        $str = str_replace(['i', 'ı', ' ü', 'ğ', 'ş', 'ö', 'ç'], ['İ', 'I', 'Ü', 'Ğ', 'Ş', 'Ö', 'Ç'], $str);

        return strtoupper($str);
    }

    public function check_alpha_numeric($str)
    {
