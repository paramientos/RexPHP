...
class io
{
     ...
     * @return bool
     */
    public function recursive_dir($folder)
    {
        if (is_dir($folder)) {
            return true;
        }
     ...
     */
    public function rmdir($dir, $virtual = false)
    {
    ...
    public function randomize_fileName($real_file_name)
    {
    ...
    public function human_size($url)
    {
        $size = filesize($url);
        if ($size <= 1024) {
            return $size.' Bytes';
     ...
     * @param type $mimetype
     */
    public function download_document($filename, $path = '', $mimetype = 'application/octet-stream')
    {
